...
def test_estimate_cohort_accepts_structured_timestamp_operators(client: TestClient) -> None:
    csv_text = (
        "user_id,event_name,event_time,country\n"
        "u1,purchase,2026-01-01 09:00:00,US\n"
        "u2,purchase,2026-01-02 09:00:00,CA\n"
    ...
    resp = client.post('/cohorts/estimate?user_id=abcdef12', json={
        'name': 'estimate-on',
        'logic_operator': 'AND',
        'conditions': [
            {
    ...
    assert resp.status_code == 200, resp.text
    assert 'estimated_users' in resp.json()
